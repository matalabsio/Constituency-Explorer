...
export default async function MandalDetailPage({
  ...
  const topVillages = getTopDhoneVillages(villages, 10);
    ...
    <DhoneShell>
      ...
      <nav aria-label="Breadcrumb" className="mb-4 text-sm text-[var(--muted)]">
        <ol className="flex items-center gap-1.5">
          <li>
            <Link href="/dhone" className={`hover:text-[var(--foreground)] ${focusRing}`}>Dhone</Link>
          </li>
          <li aria-hidden="true">/</li>
          <li>
            <Link href="/dhone/mandals" className={`hover:text-[var(--foreground)] ${focusRing}`}>Mandals</Link>
          </li>
          <li aria-hidden="true">/</li>
      ...
      <header className="mb-8">
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">
          {m.villages} villages &middot; {m.gramPanchayats} gram panchayats &middot; Nandyal district
        </p>
      </header>
        ...
        {topVillages.length > 0 && (
          <section>
            ...
            <ChartCard title="Population">
              <ColumnChart
                caption="Top villages by population"
            ...
            <div className="mt-4 overflow-hidden rounded-[var(--radius-xl)] border border-[var(--border)] bg-white">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--border)] bg-[var(--surface-muted)]">
                    ...
                    <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-[var(--muted)]">Male</th>
                    <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-[var(--muted)]">Female</th>
                ...
                <tbody>
                  {topVillages.map((v) => (
                    <tr key={v.village_name} className="border-t border-[var(--border)] hover:bg-[var(--surface-muted)] transition">
                      ...
                      <td className="px-4 py-3 text-right tabular-nums text-[var(--foreground)]">{fmt(v.population_male!)}</td>
                      <td className="px-4 py-3 text-right tabular-nums text-[var(--foreground)]">{fmt(v.population_female!)}</td>
            ...
            <p className="mt-2 text-right">
              <Link href="/dhone/villages" className="text-xs font-medium text-[var(--brand-green)] hover:underline">
      ...
      <footer className="mt-12 text-center text-xs text-[var(--muted)]">
        Source: Census 2011 &middot; nandyal.ap.gov.in
      </footer>
    </DhoneShell>
  );
}
