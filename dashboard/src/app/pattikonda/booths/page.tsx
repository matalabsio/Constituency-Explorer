...
export default function PattikondaBoothsPage() {
  ...
  const stations = useMemo(
    () =>
      POLLING_STATIONS.map((s) => ({
        ...s,
        mandal: mandalForBooth(s.name),
        village: villageForBooth(s.name) ?? "—",
      })),
    []
  );
  ...
  return (
    <PattikondaShell>
      ...
      <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        {MANDALS.map((m) => (
          <div key={m.slug} className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--card)] p-4 shadow-[var(--shadow-sm)]">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--muted)]">{m.name}</p>
            <p className="mt-1 text-xl font-semibold tabular-nums text-[var(--foreground)]">{mandalCounts[m.name] ?? 0}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row">
        <input
          type="text"
          placeholder="Search by station name, village, or part number..."
          value={query}
        ...
        <select
          ...
          className="cursor-pointer rounded-[var(--radius-md)] border border-[var(--border)] bg-white px-3 py-2.5 text-sm text-[var(--foreground)] outline-none"
        >
      ...
      <section className="mt-12">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--card)] p-5 shadow-[var(--shadow-sm)]">
            <p className="text-sm font-semibold text-[var(--foreground)]">How to download Pattikonda Voter List PDF</p>
            <ol className="mt-3 space-y-2 text-sm leading-relaxed text-[var(--muted)]">
              ...
              <li><span className="mr-1.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-[var(--surface-muted)] text-[10px] font-bold text-[var(--foreground)]">3</span>Select District: <strong>Kurnool</strong>, Assembly: <strong>Pattikonda</strong></li>
            ...
            <div className="mt-5">
              <a href="https://voters.eci.gov.in/download-eroll" target="_blank" rel="noopener noreferrer" className="inline-flex cursor-pointer items-center justify-center rounded-[var(--radius-md)] bg-[var(--accent)] px-5 py-2.5 text-sm font-medium text-white shadow-[var(--shadow-sm)] transition hover:bg-[var(--accent-hover)]">
