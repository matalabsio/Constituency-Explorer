    ...
    <PattikondaShell>
      <nav aria-label="Breadcrumb" className="mb-6 text-sm text-[var(--muted)]">
        <ol className="flex flex-wrap items-center gap-1.5">
          ...
          <li aria-hidden="true">/</li>
          <li className="font-semibold text-[var(--foreground)]">Pattikonda</li>
        </ol>
      </nav>

      <header className="mb-8">
        <p className="mark-yellow text-xs font-semibold uppercase tracking-[0.18em]">AC 142 · GEN · Kurnool</p>
        ...
        <p className="mt-3 max-w-prose text-sm leading-relaxed text-[var(--muted)]">
          Five revenue mandals, {c.totalVillages} villages, {fmt(v.totalVoters)} electors (2024).
          Telugu {d.assemblyNameTelugu}.
        </p>
      </header>

      <div className="mb-8 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard label="Registered electors" value={v.totalVoters} sub="2024 rolls (IndiaVotes)" />
        <StatCard label="2024 turnout" value={`${v.turnout2024}%`} sub={`${fmt(v.votesPolled2024)} votes polled`} />
        <StatCard label="Mandals" value={c.mandals} sub="Krishnagiri, Veldurthy, Pattikonda, Maddikera, Tuggali" />
      ...
      <div className="space-y-14">
        <Section
          ...
          description="Census 2011 population and households by mandal. Village and GP counts from Kurnool district records for the five mandals in AC 142."
        >
              ...
              {MANDALS.map((m) => (
                <tr key={m.slug}>
                  <td className="font-medium">
                    <Link
                      href={`/pattikonda/mandals/${m.slug}`}
        ...
        <Section
          id="electorate"
          title="Electorate"
          description={`${fmt(v.totalVoters)} registered electors on 2024 assembly rolls (${v.source}). Rural ${v.ruralPercent}%, urban ${v.urbanPercent}%. Gender-split rolls from other hosts are not mixed with this total.`}
        >
          <ChartCard title="Rural / urban electors">
            <PieChart
              caption={`Electors: rural ${v.ruralPercent}%, urban ${v.urbanPercent}%`}
              slices={[
                { label: "Rural", value: v.ruralPercent || 1, color: BAR.green },
                { label: "Urban", value: v.urbanPercent, color: BAR.red },
              ]}
            />
          </ChartCard>
          <dl className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: "Electors (2024)", value: fmt(v.totalVoters) },
              { label: "Votes polled", value: fmt(v.votesPolled2024) },
              { label: "Turnout", value: `${v.turnout2024}%` },
        ...
        <Section id="facts" title="Assembly facts">
          <dl className="grid gap-px overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--border)] sm:grid-cols-2">
            {[
              ...
              { label: "PIN (Pattikonda HQ)", value: PROFILE.pinCode },
        ...
        <Section
          ...
          description={`Aggregated from the five mandals. ${fmt(c.totalPopulation)} people, ${femalesPer1000} females per 1,000 males.`}
        >
        ...
        <Section id="election-2024" title="2024 assembly result" description="TDP won Pattikonda from YSRCP. Candidate votes from ECI S01-142; electors and turnout from IndiaVotes.">
        ...
        <Section id="history" title="Election history" description="Assembly results with turnout and margin where recorded. 2019 electors are from that election, not mixed with 2024 rolls.">
          <TableWrap caption="Pattikonda assembly election history">
            <thead>
              <tr>
                <th>Year</th>
      ...
      <footer className="mt-16 border-t border-[var(--border)] pt-6 pb-8 text-xs text-[var(--muted)]">
        <p>
          Sources: Election Commission of India, Census 2011, kurnool.ap.gov.in, voterslist.in, villageinfo.in, IndiaVotes.
          Population figures are Census 2011; elector figures are 2024 rolls.
        </p>
      </footer>
    </PattikondaShell>
  );
