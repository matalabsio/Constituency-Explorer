...
export default function PattikondaMandalsPage() {
  ...
  return (
    <PattikondaShell>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {MANDALS.map((m) => (
          <Link
            ...
            className="group cursor-pointer rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--card)] p-6 shadow-[var(--shadow-sm)] transition hover:border-[var(--brand-green)] hover:shadow-md"
          >
            <p className="text-lg font-semibold text-[var(--foreground)] transition group-hover:text-[var(--brand-green)]">{m.name}</p>
      ...
      <footer className="mt-12 text-center text-xs text-[var(--muted)]">
        Source: Census 2011 &middot; kurnool.ap.gov.in &middot; villageinfo.in
      </footer>
    </PattikondaShell>
  );
}
